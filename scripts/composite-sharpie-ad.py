#!/usr/bin/env python3
"""
Composite the sharpie-chop ad: Flux photo + PIL text overlay.
Headline placed on dark wood at top, CTA on dark wood at bottom.
"""
import os
...
WHITE = (255, 255, 255)
INK = (24, 28, 38)
MUTED = (155, 145, 130)  # warmer muted for photo overlays
RED = (193, 60, 55)
...
def composite_sharpie():
    src = os.path.join(OUT, "ai", "sharpie-moment-flux.png")
    ...
    # Top dark gradient — darken existing dark wood for headline legibility
    top_overlay = Image.new("RGBA", (W, 280), (0, 0, 0, 0))
    od = ImageDraw.Draw(top_overlay)
    for y in range(280):
        alpha = int(180 * (1 - y / 280) ** 1.8)
    ...
    # Top eyebrow with orange dot
    d.ellipse([GUT, 50, GUT + 11, 61], fill=ORANGE)
    eb = font(BOLD, 17)
    tracked(d, GUT + 22, 50, "FOR NC WHOLESALERS  ·  THE CHOP MOMENT", eb, ORANGE, spacing=3)

    # Big serif headline on dark wood
    fh = font(GEORGIA_BOLD, 64)
    d.text((GUT, 92), "The buyer brought", font=fh, fill=CREAM)
    d.text((GUT, 174), "a sharpie.", font=fh, fill=CREAM)

    # Tiny subhead under headline
    fk = font(REG, 17)
    d.text((GUT, 254), "Stop getting chopped at the closing table.",
           font=fk, fill=MUTED)
    ...
    out_path = os.path.join(OUT, "ad-sharpie-pain.png")
    img.save(out_path)
