...
import { investorActionRoutes } from '../routes';

export function generateStaticParams() {
  return investorActionRoutes.map((route) => ({ slug: route.slug }));
}

export default function InvestorActionPage({ params }: { params: { slug: string } }) {
  const route = investorActionRoutes.find((entry) => entry.slug === params.slug);
  if (!route) notFound();
  ...
  const primaryHref =
    route.mode === 'pricing'
      ...
      : route.mode === 'quote'
        ? '/#contact'
        : '/recurring-support#investors';

  const primaryLabel = route.cta;
  const secondaryHref = route.mode === 'subscribe' ? '/recurring-support' : '/services/investors';
  const secondaryLabel = route.mode === 'subscribe' ? 'See All Support Plans' : 'Back to Investor Services';
        ...
        <div className="relative z-10 container-pro">
          <div className="max-w-4xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={primaryHref} className="inline-flex min-w-[220px] items-center justify-center rounded-full bg-orange px-6 py-3.5 text-sm font-semibold text-white transition-all hover:bg-orange-500">
                {primaryLabel}
              </Link>
              <Link href={secondaryHref} className="inline-flex min-w-[220px] items-center justify-center rounded-full border border-white/20 px-6 py-3.5 text-sm font-semibold text-white transition-all hover:border-orange hover:text-orange-200">
                {secondaryLabel}
              </Link>
            </div>
          </div>
      ...
      <section className="border-b border-stone-200 bg-white py-14 sm:py-16">
        <div className="container-pro grid gap-6 lg:grid-cols-[0.72fr_1.28fr]">
          ...
          <div className="rounded-[24px] border border-stone-200 bg-white p-6 shadow-elev-1">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={primaryHref} className="inline-flex items-center justify-center rounded-full bg-orange px-6 py-3 text-sm font-semibold text-white transition hover:bg-orange-500">
                {primaryLabel}
              </Link>
              <Link href="/#contact" className="inline-flex items-center justify-center rounded-full border border-stone-300 bg-white px-6 py-3 text-sm font-semibold text-navy transition hover:border-orange hover:text-orange">
                Contact Southern Cities
              </Link>
            </div>
          </div>
        </div>
