            ...
            <div className="hidden lg:flex items-center gap-0.5 xl:gap-1">
              {PRIMARY_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="px-2 xl:px-2.5 py-2 rounded-lg text-[12.5px] xl:text-[13px] font-medium text-white/68 hover:text-white transition-colors duration-200 whitespace-nowrap"
                >
                  {link.label}
                </Link>
              ))}
              <CartNavLink compact className="px-2 xl:px-2.5 py-2 rounded-lg text-[12.5px] xl:text-[13px] font-medium text-white/68 hover:text-white transition-colors duration-200 whitespace-nowrap inline-flex" />
              <a
                href="https://clients.southerncitiesconstruction.com"
                target="_blank"
                rel="noopener noreferrer"
                className="ml-1 inline-flex items-center gap-1.5 rounded-full border border-white/12 bg-white/[0.05] hover:bg-white/[0.1] text-white px-3 xl:px-3.5 py-2 text-[12px] xl:text-[12.5px] font-semibold transition-all duration-200 whitespace-nowrap"
              >
                Portal
              ...
              <Link
                href="/#contact"
                className="ml-1 inline-flex items-center gap-1.5 rounded-full bg-orange hover:bg-orange-500 text-white px-4 xl:px-4.5 py-2 text-[12px] xl:text-[12.5px] font-semibold shadow-glow-orange transition-all duration-200 whitespace-nowrap"
              >
                Request Quote
              </Link>
            </div>
